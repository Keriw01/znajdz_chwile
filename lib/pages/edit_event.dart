...
//odczyt wszystkich powiadomien
  ...
  Widget build(BuildContext context) {
    return Consumer<EventsProvider>(builder: (context, eventProvider, child) {
      dropdownValue = eventProvider.tagFindName(widget.event.tagId);
      return Scaffold(
                  ...
                  onPressed: () async {
                    if (formKey.currentState!.validate()) {
                      Event eventModel = Event(
                          ...
                          eventIsDone: widget.event.eventIsDone,
                          eventNotification: _eventHaveNotification);
                      updateEventToDatabase(eventModel).then((result) {
                        if (result == true) {
                          eventProvider.updateEvent(eventModel);
                          if (_eventNotification == true) {
                            updateNotifiaction(widget.event);
                          }
                        } else {
                          Fluttertoast.showToast(msg: "Błąd, spróbuj ponownie");
                        }
